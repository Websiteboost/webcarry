# Website Configuration - WebCarry

## Home Information

### Main Title
WebCarry Gaming Services

### Subtitle
Your trusted platform for professional gaming services

### Featured Categories
- MMO Boosting
- Ranked Services
- Power Leveling
- Achievement Hunting

---

## Available Games

### Game 1
- **ID**: game-1
- **Título**: Nexus Warriors
- **Categoría**: MMO
- **Imagen**: /images/games/nexus-warriors.jpg

### Game 2
- **ID**: game-2
- **Title**: Void Legends
- **Category**: MOBA
- **Image**: /images/games/void-legends.jpg

### Game 3
- **ID**: game-3
- **Title**: Eternal Realms
- **Category**: RPG
- **Image**: /images/games/eternal-realms.jpg

### Game 4
- **ID**: game-4
- **Title**: Cyber Strike
- **Category**: FPS
- **Image**: /images/games/cyber-strike.jpg

---

## Service Categories

### Category 1: Power Leveling
- **ID**: power-leveling
- **Name**: Power Leveling
- **Description**: Fast and efficient character leveling services to reach maximum level quickly
- **Icon**: Zap

### Category 2: Ranked Boosting
- **ID**: ranked-boosting
- **Name**: Ranked Boosting
- **Description**: Professional rank climbing services by experienced players to reach your desired tier
- **Icon**: Trophy

### Category 3: Achievement Hunting
- **ID**: achievement-hunting
- **Name**: Achievement Hunting
- **Description**: Complete difficult achievements and unlock exclusive rewards and titles
- **Icon**: Award

### Category 4: Gear Farming
- **ID**: gear-farming
- **Name**: Gear Farming
- **Description**: Obtain the best equipment and legendary items with optimized farming strategies
- **Icon**: Shield

### Category 5: Dungeon Carries
- **ID**: dungeon-carries
- **Name**: Dungeon Carries
- **Description**: Expert dungeon runs with guaranteed loot and efficient completion times
- **Icon**: Swords

### Category 6: PvP Training
- **ID**: pvp-training
- **Name**: PvP Training
- **Description**: Learn advanced PvP techniques and strategies from professional players
- **Icon**: Sword

### Category 7: Raid Completion
- **ID**: raid-completion
- **Name**: Raid Completion
- **Description**: Full raid clears with experienced teams, including all bosses and exclusive rewards
- **Icon**: Users

### Category 8: Character Customization
- **ID**: character-customization
- **Name**: Character Customization
- **Description**: Transform your character with premium cosmetics, skins, and exclusive appearance items
- **Icon**: Palette

---

## Services

### Power Leveling Service 1
- **ID**: pl-1
- **Title**: Level 1-50 Express
- **Category**: power-leveling
- **Price**: 25
- **Image**: /images/services/leveling-1.jpg
- **Description**:
  - Fast boost from level 1 to 50
  - Completed in 24-48 hours
  - Basic equipment included

### Power Leveling Service 2
- **ID**: pl-2
- **Title**: Max Level Premium
- **Category**: power-leveling
- **Price**: 45
- **Image**: /images/services/leveling-2.jpg
- **Description**:
  - Maximum level guaranteed
  - Epic equipment included
  - 24/7 support

### Ranked Boosting Service 1
- **ID**: rb-1
- **Title**: Bronze to Gold
- **Category**: ranked-boosting
- **Price**: 35
- **Image**: /images/services/ranked-1.jpg
- **Description**:
  - Guaranteed climb to Gold
  - Professional players
  - Live streaming available

### Ranked Boosting Service 2
- **ID**: rb-2
- **Title**: Platinum Package
- **Category**: ranked-boosting
- **Price**: 65
- **Image**: /images/services/ranked-2.jpg
- **Description**:
  - Reach Platinum rank
  - Priority queue
  - Coaching included

### Achievement Hunting Service 1
- **ID**: ah-1
- **Title**: Achievement Bundle Basic
- **Category**: achievement-hunting
- **Price**: 20
- **Image**: /images/services/achievement-1.jpg
- **Description**:
  - 10 achievements of your choice
  - Completed in 3-5 days
  - Detailed guide included

### Achievement Hunting Service 2
- **ID**: ah-2
- **Title**: 100% Completion
- **Category**: achievement-hunting
- **Price**: 75
- **Image**: /images/services/achievement-2.jpg
- **Description**:
  - All game achievements
  - Rare titles and rewards
  - Completion guarantee

### Gear Farming Service 1
- **ID**: gf-1
- **Title**: Epic Gear Set
- **Category**: gear-farming
- **Price**: 30
- **Image**: /images/services/gear-1.jpg
- **Description**:
  - Complete epic gear set
  - Optimized stats
  - Efficient farming

### Gear Farming Service 2
- **ID**: gf-2
- **Title**: Legendary Weapons
- **Category**: gear-farming
- **Price**: 50
- **Image**: /images/services/gear-2.jpg
- **Description**:
  - Exclusive legendary weapons
  - Premium enchants
  - Improved drop rate

### Dungeon Carries Service 1
- **ID**: dc-1
- **Title**: Normal Dungeon Pack
- **Category**: dungeon-carries
- **Price**: 15
- **Image**: /images/services/dungeon-1.jpg
- **Description**:
  - 5 normal dungeons
  - All loot reserved for you
  - Completed in 2 hours

### Dungeon Carries Service 2
- **ID**: dc-2
- **Title**: Mythic+ Bundle
- **Category**: dungeon-carries
- **Price**: 55
- **Image**: /images/services/dungeon-2.jpg
- **Description**:
  - Mythic+15 dungeons
  - Experienced group
  - Guaranteed loot

### PvP Training Service 1
- **ID**: pvp-1
- **Title**: PvP Basics
- **Category**: pvp-training
- **Price**: 25
- **Image**: /images/services/pvp-1.jpg
- **Description**:
  - PvP combat fundamentals
  - 5 training sessions
  - Replay analysis

### PvP Training Service 2
- **ID**: pvp-2
- **Title**: Arena Mastery
- **Category**: pvp-training
- **Price**: 60
- **Image**: /images/services/pvp-2.jpg
- **Description**:
  - Advanced arena training
  - Team strategies
  - Personalized coaching

### Raid Completion Service 1
- **ID**: rc-1
- **Title**: Normal Raid Clear
- **Category**: raid-completion
- **Price**: 40
- **Image**: /images/services/raid-1.jpg
- **Description**:
  - Full raid on normal difficulty
  - All loot reserved
  - Organized group

### Raid Completion Service 2
- **ID**: rc-2
- **Title**: Heroic Full Clear
- **Category**: raid-completion
- **Price**: 80
- **Image**: /images/services/raid-2.jpg
- **Description**:
  - Full heroic raid
  - Mounts and titles included
  - Top guild carry

### Character Customization Service 1
- **ID**: cc-1
- **Title**: Appearance Makeover
- **Category**: character-customization
- **Price**: 10
- **Image**: /images/services/custom-1.jpg
- **Description**:
  - Complete appearance change
  - Style consultation
  - Exclusive skins

### Character Customization Service 2
- **ID**: cc-2
- **Title**: Premium Cosmetics
- **Category**: character-customization
- **Price**: 35
- **Image**: /images/services/custom-2.jpg
- **Description**:
  - Premium cosmetics pack
  - Seasonal items
  - Special effects

---

## Payment Methods

### PayPal
- **ID**: paypal
- **Name**: PayPal
- **Icon**: 💳

### Credit Card
- **ID**: credit-card
- **Name**: Visa/Mastercard
- **Icon**: 💳
